...
class Petsc(Package, CudaPackage, ROCmPackage):
    ...
    variant("ml", default=False, description="Actives support for ML")
    variant(
    ...
    # Other dependencies
    depends_on("chaco-petsc", when="+chaco")
    ...
    #depends_on("trilinos@develop+mpi+ml", when="@main+ml+mpi")
    depends_on("mkl", when="+mkl-pardiso")
    depends_on("ml+mpi", when="+ml+mpi")
